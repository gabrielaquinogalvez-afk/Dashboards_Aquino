...
<head>
    <meta charset="utf-8">
    <title>🌞 Dashboard Solar RD 5.4</title>
...
<div class="info-box" id="infoBox">Selecciona una provincia para ver información.</div>
...
async function loadRadiation(){
    ...
    const res = await fetch(`/radiation?provincia=${provincia}`);
    ...
    chart = new Chart(ctx, {
        ...
        options:{
            ...
            scales:{y:{beginAtZero:true}, x:{title:{display:true, text:'Hora'}}}
        }
    });
}

function downloadCSV(){
    if(!currentData){alert('Primero carga los datos'); return;}
    let csv="Hora,Radiación Solar (W/m²)\\n";
    for(let i=0;i<currentData.hours.length;i++){
        csv+=`${currentData.hours[i]},${currentData.values[i]}\\n`;
    }
    const blob=new Blob([csv],{type:'text/csv'});
    const url=URL.createObjectURL(blob);
    const a=document.createElement('a'); a.href=url; a.download='radiacion.csv'; a.click();
}

// Cargar automáticamente al inicio
window.onload = loadRadiation;
</script>
</body>
</html>
"""
...
async def home():
    options = "".join([f"<option value='{p}'>{p}</option>" for p in PROVINCIAS])
    return HTML_TEMPLATE.format(options=options)

@app.get("/radiation")
async def get_radiation(provincia: str):
    if provincia not in PROVINCIAS: return {"error":"Provincia no válida"}
    ...
    return {"hours":[h.split("T")[1] for h in hours],"values":values,"info":{"max":max_rad,"avg":avg_rad,"date":date}}
